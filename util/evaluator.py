...
def print_statistics(gold,predicted,run) :
    ...
    for kind in sorted(gold) :
        ...
        run["eval/f1_score_"+kind] = F1
        run["eval/precision_"+kind] = P
        run["eval/recall_"+kind] = R
    ...
    run["eval/f1_score"] = sF1
    run["eval/precision"] = sP
    run["eval/recall"] = sR
    run.wait()
    return run           
...
def evaluate(task, golddir, outfile, run):
    ...
    run = print_statistics(gold,predicted,run)
    return run
